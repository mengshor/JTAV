...
from keras.layers import Permute, Input, Dense, Lambda, BatchNormalization, Concatenate, Flatten, MaxPool1D
...
if __name__ == '__main__':
    ...
    caption_input = Input(shape=input_caption_dim)
    caption = MaxPool1D(pool_size=5)(caption_input)
    ...
    ################################################
    m_1 = MatMul((300, 1), (1, 300))([audio, caption])
    m_2 = MatMul((300, 1), (1, 300))([audio, image])
    m_3 = MatMul((300, 1), (1, 300))([caption, image])
    ...
    model = Model([image_input, caption_input, audio_input], y)
    model.summary()
    batch_size = 32
    model.compile(loss=loss, optimizer=opt)
    model.fit([train_image, train_caption, train_audio], train_label, verbose=2, batch_size=batch_size, epochs=5)
    results = model.predict_generator(generate_batch_data([test_image, test_caption], [test_audio], predict_batch),
                                          steps=predict_steps)
    ...
    print('########### {} ###########'.format('image caption audio'))
    print(scores)
